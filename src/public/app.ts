...
const ZS: Record<string, ZoneStyle> = {
...
let drawingZone: { x: number; y: number; w: number; h: number } | null = null;
let isDrawing = false;
...
let pathPreview: Point[] = [];
let pendingZoneType: 'nogo' | 'clean' | 'sched' | 'custom' = 'nogo';

let camOpen = false;
let camExpanded = false;
let camSource: 'webcam' | 'arduino' | 'mjpeg' = 'webcam';
let mediaStream: MediaStream | null = null;
let mjpegTimer: ReturnType<typeof setInterval> | null = null;
let fpsTimer: ReturnType<typeof setInterval> | null = null;
let visionTimer: ReturnType<typeof setInterval> | null = null;
let fpsFrames = 0;
let fpsLast = Date.now();
let demoDetections: Detection[] = [];
...
function mpos(e: MouseEvent | TouchEvent): Point {
...
function inp(p: Point, r: { x: number; y: number; w: number; h: number }): boolean {
...
function showTab(n: string): void {
...
function pickType(t: string, el: HTMLElement): void {
  pendingZoneType = t as 'nogo' | 'clean' | 'sched' | 'custom';
  document.querySelectorAll('.type-opt').forEach(o => (o as HTMLElement).className = 'type-opt');
  el.className = 'type-opt sel-' + t;
}

function setTypeUI(t: string): void {
  pendingZoneType = t as 'nogo' | 'clean' | 'sched' | 'custom';
  const ts = ['nogo', 'clean', 'sched', 'custom'];
  document.querySelectorAll('.type-opt').forEach((o, i) => {
    (o as HTMLElement).className = 'type-opt' + (ts[i] === t ? ' sel-' + t : '');
  });
}

function confirmZone(): void {
  if (!drawingZone) return;
  const nameInput = document.getElementById('zoneName') as HTMLInputElement;
...
function togD(id: number, i: number): void {
  const s = schedules.find(x => x.id === id);
  if (s) {
...
function updS(id: number, k: string, v: string): void {
  const s = schedules.find(x => x.id === id);
  if (s) {
    (s as any)[k] = k === 'interval' ? parseInt(v) || 0 : v;
    renderSchedules();
  }
}

function delS(id: number): void {
  schedules = schedules.filter(s => s.id !== id);
  renderSchedules();
}

function nxRun(s: Schedule): string {
  const now = new Date();
...
function getBounds(): { minX: number; maxX: number; minY: number; maxY: number } {
  if (!roomPolygon.length) return { minX: 50, maxX: mapW - 50, minY: 50, maxY: mapH - 50 };
  return {
...
function previewPath(): void {
  ...
  if (selectedPathMode === 'zigzag') {
    ...
    let y1 = b.maxY - s;
    while (x0 < x1 && y0 < y1) {
      for (let x = x0; x <= x1; x += s) pathPreview.push({ x, y: y0 });
      for (let y = y0 + s; y <= y1; y += s) pathPreview.push({ x: x1, y });
      for (let x = x1 - s; x >= x0; x -= s) pathPreview.push({ x, y: y1 });
      for (let y = y1 - s; y > y0; y -= s) pathPreview.push({ x: x0, y });
      x0 += s;
      x1 -= s;
      y0 += s;
      y1 -= s;
    ...
    for (let x = b.minX + s; x <= b.maxX - s; x += s) pathPreview.push({ x, y: b.minY + s });
    for (let y = b.minY + s; y <= b.maxY - s; y += s) pathPreview.push({ x: b.maxX - s, y });
    for (let x = b.maxX - s; x >= b.minX + s; x -= s) pathPreview.push({ x, y: b.maxY - s });
    for (let y = b.maxY - s; y >= b.minY + s; y -= s) pathPreview.push({ x: b.minX + s, y });
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (msg.event === 'sensor_update' && msg.data?.position) {
          const p = msg.data.position as { x: number; y: number };
        ...
        if (msg.event === 'vision_detections' && Array.isArray(msg.data?.detections)) {
          demoDetections = msg.data.detections.map((d: any) => ({
...
function sendWS(event: string, data?: Record<string, unknown>): void {
...
canvas.addEventListener('mousedown', (e: MouseEvent) => {
  ...
  if (['nogo', 'clean', 'sched', 'custom'].includes(tool)) {
    isDrawing = true;
    dragStart = p;
    pendingZoneType = tool as 'nogo' | 'clean' | 'sched' | 'custom';
...
canvas.addEventListener('mouseup', () => {
  if (isDrawing && drawingZone && drawingZone.w > 10 && drawingZone.h > 10) {
    ...
    (document.getElementById('zoneName') as HTMLInputElement).value = '';
...
const camVideo = document.getElementById('camVideo') as HTMLVideoElement;
const camImg = document.getElementById('camImg') as HTMLImageElement;
const camOvCanvas = document.getElementById('camOverlayCanvas') as HTMLCanvasElement;
const camNoFeed = document.getElementById('camNoFeed')!;

function initCameraDrag(): void {
...
let camAiMode = false;
let camStreamImg: HTMLImageElement | null = null;

function toggleCam(): void {
  camOpen = !camOpen;
  const btn = document.getElementById('btnCamera');
  const placeholder = document.getElementById('camPlaceholder');
  const streamImg = document.getElementById('camStream') as HTMLImageElement;
  
  if (btn) btn.classList.toggle('primary', camOpen);
...
function setCamMode(mode: string): void {
  ...
  if (btnRaw) btnRaw.classList.toggle('primary', !camAiMode);
  if (btnAi) btnAi.classList.toggle('primary', camAiMode);
  if (camLabel) camLabel.textContent = camAiMode ? 'AI VISION' : 'LIVE';
...
function setSource(src: string): void {
  stopAllFeeds();
  camSource = src as 'webcam' | 'arduino' | 'mjpeg';
  ...
  if (src === 'arduino') (document.getElementById('camUrl') as HTMLInputElement).placeholder = 'http://192.168.x.x/cam';
  if (src === 'mjpeg') (document.getElementById('camUrl') as HTMLInputElement).placeholder = 'http://host/stream.mjpg';
...
function connectExternal(): void {
  const url = (document.getElementById('camUrl') as HTMLInputElement).value.trim();
  if (!url) {
    showCamErr('Enter a URL first.');
    return;
  }
  stopAllFeeds();
  const btn = document.getElementById('camConnBtn') as HTMLButtonElement;
...
function stopAllFeeds(): void {
  ...
  camImg.onerror = null;
  camNoFeed.style.display = 'flex';
  if (mjpegTimer) clearInterval(mjpegTimer);
...
function drawVisionOv(): void {
  ...
  demoDetections.forEach(d => {
    ...
    [
      ...
      c.beginPath();
      c.moveTo(cx as number, cy as number);
      c.lineTo(cx as number + (sx as number) * cl, cy as number);
      c.stroke();
      c.beginPath();
      c.moveTo(cx as number, cy as number);
      c.lineTo(cx as number, cy as number + (sy as number) * cl);
      c.stroke();
    });
    c.fillStyle = col + 'cc';
...
function startVision(): void {
  ...
  visionTimer = setInterval(() => {
    ...
    el.innerHTML = '<span>Vision:</span>' + demoDetections.map(d =>
      `<span class="vtag ${d.label === 'Person' ? 'vtag-obs' : 'vtag-obj'}">${d.label.toUpperCase()}</span>`
    ).join('');
  }, 1200);
}
...
function toggleVision(): void {
  ...
  if (ws?.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ event: 'vision_enable', data: { enabled: visionEnabled } }));
  }
}

function showCamErr(msg: string): void {
  camNoFeed.style.display = 'flex';
...
(window as any).saveMap = saveMap;
(window as any).startScan = startScan;
(window as any).rescan = rescan;
(window as any).toggleCam = toggleCam;
(window as any).toggleCamExpand = toggleCamExpand;
(window as any).setSource = setSource;
(window as any).connectExternal = connectExternal;
(window as any).takeSnapshot = takeSnapshot;
(window as any).showTab = showTab;
(window as any).pickType = pickType;
(window as any).confirmZone = confirmZone;
(window as any).cancelZone = cancelZone;
(window as any).deleteZone = deleteZone;
(window as any).selZ = selZ;
(window as any).addSchedule = addSchedule;
(window as any).renderSchedules = renderSchedules;
(window as any).togS = togS;
(window as any).togD = togD;
(window as any).updS = updS;
(window as any).delS = delS;
